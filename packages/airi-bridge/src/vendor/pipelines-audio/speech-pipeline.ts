 ...
 * Source: packages/pipelines-audio/src/speech-pipeline.ts
 ...
 *
 * Verbatim. Carries PROTOCOL.md §6 invariants 2 and 3: TTS runs `ttsMaxConcurrent`-way
 * concurrent while `scheduleCompletedRequests()` releases playback strictly in text order,
 * storing `null` for a failed segment so the sequence gate advances instead of deadlocking;
 * a segment's special token is emitted only after `waitForPlayback(item)` resolves.
 */
