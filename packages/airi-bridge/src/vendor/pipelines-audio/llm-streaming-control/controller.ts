 ...
 * Source: packages/pipelines-audio/src/llm-streaming-control/controller.ts
 ...
 *
 * Deviation: two type-only annotations added so the file compiles under
 * `strict: true` (upstream's tsconfig leaves `strict` off). Marked inline
 * with `DEVIATION:`. No runtime behaviour changed.
 */
...
function registerHandler<TPayload extends Record<string, unknown>>(
  ...
  // DEVIATION: `as unknown as` widened from upstream's `as`. Upstream compiles with
  // `strictFunctionTypes` off (bivariant parameters); Bright compiles with `strict: true`.
  // Runtime behaviour is identical.
  set.add(handler as unknown as LlmStreamingControlCallHandler)

  return () => {
    set!.delete(handler as unknown as LlmStreamingControlCallHandler) // DEVIATION: see above
...
export function createStreamingControlParser(
  ...
  function createTurnApi(
    ...
    return {
      ...
      // DEVIATION: parameter types written explicitly. Upstream relies on contextual
      // typing that only works with `noImplicitAny` off.
