 ...
 * Source: packages/stage-ui-live2d/src/composables/live2d/motion-manager.test.ts (v0.11.3 @ b230e16)
 * Copyright (c) 2024-present Moeru AI Project AIRI Team. Licensed under the MIT License.
 *
 * Upstream test, kept as regression coverage for the ported motion manager.
 * Only two lines changed: `ref` now comes from our `./ref` shim instead of Vue, and
 * the mocked module is `./idle-eye-focus` (upstream's `./animation`). Every
 * assertion is untouched — this is the evidence the port preserved behaviour.
 */
