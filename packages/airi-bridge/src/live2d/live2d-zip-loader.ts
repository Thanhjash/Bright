 ...
 * Source: packages/stage-ui-live2d/src/utils/live2d-zip-loader.ts (v0.11.3 @ b230e16)
 * Copyright (c) 2024-present Moeru AI Project AIRI Team. Licensed under the MIT License.
 *
 * Copied VERBATIM.
 *
 * SIDE EFFECTS ON IMPORT. This module patches pixi-live2d-display's static
 * `ZipLoader` and `FileLoader` in place. Import it exactly once, before loading any
 * model, and only when the model source is a `.zip` or a directory upload. A plain
 * `.model3.json` URL does not need it, which is why `createLive2DStage` does not
 * import it for you.
 *
 * Importing it also supersedes most of AIRI's `patches/pixi-live2d-display.patch`:
 * that patch teaches the unpatched loaders to skip `items_pinned_to_model.json`,
 * which the `createSettings` overrides below already do. Bright therefore runs
 * pixi-live2d-display 0.4.0 unpatched.
 */
