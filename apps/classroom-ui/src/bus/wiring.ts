...
import { observeOutputActivity, publishOutputActivity } from '../speech/outputActivity'
...
export function connectBusToStore(bus: Bus): Unsubscribe {
  ...
  const ownsAudio = bus.role === 'stage'
  const textByTurn = new Map<string, string>()
  const releaseSpeechOutput = ownsAudio
    ? configureSpeechOutput({
        onPlaybackStarted: (speechTurnId) => {
        ...
        onPlaybackFinished: (speechTurnId, status, reason, reportToCore = true) => {
  ...
  const offs: Unsubscribe[] = [
    ...
    bus.onReset((reason) => {
      if (ownsAudio) {
        ...
        stopSpeaking('transport-reset', false)
      }
    ...
    bus.on('scene.snapshot', (payload) => store().applySnapshot(payload)),
