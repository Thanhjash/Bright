...
The input checkout must be clean and exactly at the manifest commit. This
script applies the tracked patch itself; it never builds an unknown dirty
vendor tree. The produced wheel checksum is written beside the wheel and must
be promoted into the deployment bundle before installation.
"""
...
def main() -> int:
    ...
    dirty = _run(["git", "status", "--porcelain"], cwd=repo, capture=True)
    if dirty:
        raise SystemExit(
            "refusing dirty Hermes checkout; use a clean checkout at the pinned commit"
        )
    ...
    for entry in manifest["patches"]:
        ...
        _run(["git", "apply", "--check", str(patch)], cwd=repo)
    ...
    _run(build_command, cwd=repo, env=build_env)
