# RUNTIME TOPOLOGY, PACKAGING & DIRECTORY LAYOUT

**Date:** 2026-08-11
**Governed by:** [north star](../NORTH-STAR.md)

> **Still current** for packaging, ports, the two screens, the Stage layer order
> and the boot sequence.
>
> **Superseded** on interaction: any passage describing an adult starting the
> class or holding a button to talk is historical. The room opens itself — see
> [decisions/2026-08-18-room-runs-itself.md](../decisions/2026-08-18-room-runs-itself.md).
>
> **Superseded** on the content pipeline (2026-08-18): `lesson_run.json`,
> `runs/`, `lesson-lint/`, `lesson-compile/` and `authoring-studio/` describe a
> compile-a-lesson-graph toolchain that has been **deleted**. The teacher reads
> markdown from `content/library/` and decides for herself; there is nothing to
> compile. `data/schemas/` as a validation discipline still stands — but the
> `lesson_run.json` schema it names does not exist any more.
...
`Take over` is not a nice-to-have. Codex ranked loss of teacher control as risk #2 for this product. The console must let a teacher steer or stop the agent at any moment, with one tap, without understanding anything about the system. See [execution plan](../archive/execution-plan.md).
...
| Iteration speed | hot reload beats a native rebuild loop, and content authoring will dominate this project ([execution plan](../archive/execution-plan.md)) |
...
| Updates | offline. USB or scheduled sync visit ([open questions](../archive/open-questions.md) Q5) |
