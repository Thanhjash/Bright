# REFERENCE REPOS: what to take, what to leave
...
- `packages/stage-ui/src/libs/speech/streaming-pipeline.ts` — requires AIRI server-runtime (`/api/v1/audio/speech/ws`, auth token, `STREAMING_TTS_UPSTREAM`). See [fact check](../decisions/fact-check-gpt-brief.md) #7
