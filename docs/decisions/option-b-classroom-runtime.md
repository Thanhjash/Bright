# DECISION: Option B classroom runtime
...
programmatic API. **Hermes is the teacher.** Classroom Core is the classroom
OS (session, I/O, semantic DB, safety, restart). Hermes acts only through a
Bright-owned MCP surface — a small set of library/board/say/evidence tools,
not a single `classroom_propose_move` menu. AIRI renders speech and
behaviour; it is not a second agent runtime.

Owner correction 2026-08-16: [teacher-agent-not-cassette.md](teacher-agent-not-cassette.md).
The process topology (sidecar, MCP, Stage audio, dedicated ASR) stays.
The cassette (`lesson_run.json` as teacher when the LLM dies) does not.
...
| Concern | Sole owner |
|---|---|
| pedagogy, pacing, what to teach next | Hermes |
| durable learner mastery / observations | Bright database (via agent tools) |
| session, board/mic I/O, restart | Classroom Core |
| ASR transcript production | Bright speech service |
| physical classroom audio | Stage |
| avatar, lipsync, timed emotion | AIRI bridge |

There are no dual writers of durable memory. Hermes decides; Core applies
typed tool side-effects or rejects them. The retired one-tool
`classroom_propose_move` contract was a harness probe, not the teacher
surface. Live tools are the library + present + say + evidence + response
set in the 2026-08-16 correction. Generic Hermes filesystem/browser/cron
stay off. Invalid tool args fail closed — they do not start a cassette.
