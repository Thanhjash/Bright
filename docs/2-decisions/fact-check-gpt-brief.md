# 02 — FACT CHECK of the GPT "MASTER CONTEXT" brief
...
→ **Architecture decision in [architecture](../3-design/architecture.md): use the REST + segmenter path with our own TTS service. Do not pull in AIRI server-runtime.**
...
What to actually take: [reusing AIRI](../3-design/reusing-airi-and-friends.md).
...
Direct consequence: GPT's section 43 proposal ("use Gemma audio to verify ambiguous utterances") **may be dead on the Intel box**. Do not design anything that depends on it. → Spike SP-1 in [open questions](../4-build/open-questions.md).
