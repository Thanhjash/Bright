...
> live in [Option B classroom runtime](../../decisions/option-b-classroom-runtime.md)
> and the [North Star](../../NORTH-STAR.md). When this report conflicts with
...
1. `NORTH-STAR.md` is the governing product doctrine.
2. `decisions/` records choices and source-based fact checks.
3. `design/` describes the target architecture.
4. `archive/` contains several snapshots from different moments on 2026-08-11.
5. `research/` contains later reviews and explicit corrections to earlier claims.
