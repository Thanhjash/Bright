...
**Date:** 2026-08-11
**Rule:** nothing in `../3-design/architecture.md` gets implemented past a prototype until the spike that de-risks it has run. Each spike is timeboxed and has a written kill criterion.
...
**Do:** instrument every hop with the event bus timestamps. Compare REST-segmenter TTS against AIRI's `streaming-pipeline.ts` to quantify what we gave up by not running AIRI server-runtime (see [fact check](../2-decisions/fact-check-gpt-brief.md) #7).
...
6. Map Hermes SSE events → animations per [Hermes decision](../2-decisions/hermes-over-openclaw.md)
...
Do **not** try to reverse-engineer ELSA. Establish a measurable baseline first, and only emit descriptive feedback until the scores are calibrated ([architecture](../3-design/architecture.md) §7).

### SP-9 — Identity fusion

Build the fusion spec from [architecture](../3-design/architecture.md) §5 and measure accuracy in a realistic noisy room. Component questions:
